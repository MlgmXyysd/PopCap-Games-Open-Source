...
{
public:
	virtual void onPerfStart(const char* inName) = 0;
	virtual void onPerfStop(const char* inName) = 0;
};
...
public:
	...
	*/
	WebCore(LogLevel level = LOG_NORMAL, bool enablePlugins = true, PixelFormat pixelFormat = PF_BGRA, const std::wstring& cookieStoreFile = L"", const std::wstring& cacheFilePath = L"", WebPerfApi* perfApi = 0);
	...
	*/
	...
	void reInit(const std::wstring& cookieStoreFile = L"", const std::wstring& cacheFilePath = L"");
...
protected:
	...
	Lock *eventQueueLock, *baseDirLock, *customResponsePageLock, *reinitLock;
