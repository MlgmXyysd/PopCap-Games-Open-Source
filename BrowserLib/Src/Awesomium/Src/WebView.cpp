...
/*
	...
	2009.07.21: Added postData and postSeparator arguments, for POST support
	2009.07.29: Added bgInfo argument, for transparency improvement
	2009.10.27: Added opaqueRect argument, another transparency improvement
	2009.11.10: Added renderMode support, for debugging dirtyrects
*/
...
: listener(0), dirtiness(false), enableAsyncRendering(enableAsyncRendering), coreThread(coreThread)
{
...
{
	coreThread->message_loop()->PostTask(FROM_HERE, NewRunnableMethod(viewProxy, &WebViewProxy::loadURL, url, frameName, username, password, postData, postSeparator));
}
...
void Awesomium::WebView::render(unsigned char* destination, int destRowSpan, int destDepth, Awesomium::Rect* renderedRect, Awesomium::WebView::BackgroundInfo* bgInfo, Awesomium::Rect* opaqueRect)
{
	if(enableAsyncRendering)
	{
	...
	{
		extern WebPerfApi* gPerfApi;
		if (gPerfApi)
			gPerfApi->onPerfStart("WebView::render");
		
		coreThread->message_loop()->PostTask(FROM_HERE, NewRunnableMethod(viewProxy, &WebViewProxy::renderSync, destination, destRowSpan, destDepth, renderedRect, bgInfo, opaqueRect));
		waitState->renderEvent.Wait();

		if (gPerfApi)
			gPerfApi->onPerfStop("WebView::render");
	}
}
...
void Awesomium::WebView::setRenderMode(ERenderMode renderMode)
{
	coreThread->message_loop()->PostTask(FROM_HERE, NewRunnableMethod(viewProxy, &WebViewProxy::setRenderMode, renderMode));
}
