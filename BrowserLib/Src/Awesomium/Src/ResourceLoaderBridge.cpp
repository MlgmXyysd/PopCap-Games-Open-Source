...
/*
	...
	2009.07.21: Changed request_context to use our modified
	            TestShellRequestContext specifically, rather than
	            URLRequestContext generally, for the persistent cookie store
	2009.08.04: Merged io_thread fix from Awesomium trunk
*/
...
namespace {
	...
	TestShellRequestContext* request_context = NULL;
...
void SimpleResourceLoaderBridge::Init(TestShellRequestContext* context) {
	...
	if (context) {
		...
		request_context = new TestShellRequestContext(L"");
	}
	request_context->AddRef();
	
	// xAJS begin:
	if(!coreProxyMsgLoop)
		coreProxyMsgLoop = MessageLoop::current();
	// xAJS end
}
