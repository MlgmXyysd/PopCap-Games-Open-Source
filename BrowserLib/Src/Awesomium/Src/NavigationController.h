...
/*
	...
	2009.07.21: Added postData and postSeparator fields, arguments,
	            and get-accessors, for POST support
*/
...
class NavigationEntry {
 public:
	...
	NavigationEntry(int page_id, const GURL& url, const std::wstring& title, const std::wstring& target_frame,
		const std::string& authUsername = "", const std::string& authPassword = "", const std::string& postData = "", const std::string& postSeparator = "")
	: page_id_(page_id),
	  url_(url),
	  title_(title),
	  target_frame_(target_frame),
	  authUsername(authUsername), authPassword(authPassword),
	  postData(postData), postSeparator(postSeparator) {}
	...
	const std::string& GetPostSeparator() const { return postSeparator; }
...
private:
	...
	std::string postData, postSeparator;
