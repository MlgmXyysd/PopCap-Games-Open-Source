...
Awesomium::WebPerfApi* gPerfApi = 0;

namespace Awesomium {

WebCore::WebCore(LogLevel level, bool enablePlugins, PixelFormat pixelFormat, const std::wstring& cookieStoreFile, const std::wstring& cacheFilePath, WebPerfApi* perfApi )
: pluginsEnabled(enablePlugins), pixelFormat(pixelFormat)
{
	assert(!instance);
	instance = this;
	...
	if(logOpen)
	{
		...
		logging::InitLogging(log_filename.value().c_str(),
			logging::LOG_ONLY_TO_SYSTEM_DEBUG_LOG, //level == LOG_VERBOSE ? logging::LOG_TO_BOTH_FILE_AND_SYSTEM_DEBUG_LOG : logging::LOG_ONLY_TO_FILE,
	...
	coreProxy = new WebCoreProxy(coreThread, pluginsEnabled, cookieStoreFile, cacheFilePath);
...
void WebCore::reInit(const std::wstring& cookieStoreFile, const std::wstring& cacheFilePath)
{
	AutoLock autoReinitLock(*reinitLock);

	coreProxy->reInit(cookieStoreFile, cacheFilePath);
}
