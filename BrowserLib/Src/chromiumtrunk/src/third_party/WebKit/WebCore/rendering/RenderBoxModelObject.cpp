...
// CDH: define this to enable PopCap's changes
#define POPCAP_CHROMIUM

#ifdef POPCAP_CHROMIUM
#include "style\\StyleCachedImage.h"
#include "..\\platform\\graphics\\BitmapImage.h"
#include <vector>
#include <string>

std::string g_PopCap_Chromium_BackgroundSubStr = "";
std::vector<WebCore::BitmapImage*> g_PopCap_Chromium_BackgroundImages;
std::vector<std::wstring> g_PopCap_Chromium_BackgroundImageUrls;
#endif // POPCAP_CHROMIUM
...
#ifdef POPCAP_CHROMIUM
			if (!g_PopCap_Chromium_BackgroundSubStr.empty())
			{
				StyleImage* styleImg = bgLayer->image();
				if (styleImg->isCachedImage())
				{
					StyleCachedImage* styleCachedImg = (StyleCachedImage*)styleImg;
					CachedImage* cachedImg = styleCachedImg->cachedImage();
					if (cachedImg)
					{
						String url = cachedImg->url();
						if (url.contains(g_PopCap_Chromium_BackgroundSubStr.c_str(), false))
						{
							Image* img = cachedImg->image();
							if (img->isBitmapImage())
							{
								BitmapImage* bitmapImg = (BitmapImage*)img;
								
								bool doAdd = true;
								int bgCount = g_PopCap_Chromium_BackgroundImages.size();
								for (int iBg=0; iBg<bgCount; ++iBg)
								{
									if (g_PopCap_Chromium_BackgroundImages[iBg] == bitmapImg)
									{
										doAdd = false;
										break;
									}
								...
								if (doAdd)
								{
									g_PopCap_Chromium_BackgroundImages.push_back(bitmapImg);
									g_PopCap_Chromium_BackgroundImageUrls.push_back(url.charactersWithNullTermination());
								}
							}
						}
					}
